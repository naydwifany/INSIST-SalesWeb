                ...
                fetch(`/sales/${salesId}`)
                    ...
                    .then(data => {
                        if (data) {
                            ...
                            document.getElementById('ClientPIC_Name').value = data.ClientPIC_Name;
            ...
            if (salesId) {
                ...
                if (salesForm.checkValidity()) {
                    try {
                        ...
                        if (response.ok) {
                            const result = await response.text(); // Atau response.text() jika server tidak mengembalikan JSON
                            ...
                            const error = await response.text(); // Atau response.text() jika server tidak mengembalikan JSON
                            console.error('Error:', error);
                            alert('Failed to create sales: ' + (error.message || 'Unknown error'));
                        }
                    } catch (error) {
                        // Menangani kesalahan dari fetch
                        console.error('Error:', error);
                        alert('Failed to create sales: ' + error.message);
                    }
                } else {
